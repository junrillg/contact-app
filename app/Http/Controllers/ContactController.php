...
class ContactController extends Controller
{
    ...
    public function store(Request $request)
    {
       ...
       $isValidContactData = isset($requestBody['firstName']) && isset($requestBody['lastName']);

       // validate if response body data is valid and create new Contact data
       if(!$isValidContactData) {
           return  $this->response->errorWrongArgs("firstName and lastName are required!");
       } else {
           // create new contact
           $contact = new Contact([
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!$contact) {
            ...
            $firstName = isset($firstNameData) ? $firstNameData: $contact->first_name;
